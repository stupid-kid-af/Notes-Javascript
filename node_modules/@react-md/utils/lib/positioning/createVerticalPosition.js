...
function createAnchoredAbove(config) {
    ...
    var top = (0, getCoord_1.getAboveCoord)(config);
    var actualY = "above";
    if (disableVHBounds) {
    ...
    var swappedTop = (0, getCoord_1.getBelowCoord)(config);
...
function createAnchoredTop(config) {
    ...
    var top = (0, getCoord_1.getTopCoord)(config);
    var actualY = "top";
    if (disableVHBounds || top + elHeight <= screenBottom) {
        return { actualY: actualY, top: top };
    }
    var swappedTop = (0, getCoord_1.getBottomCoord)(config);
...
function createAnchoredCenter(config) {
    ...
    var top = (0, getCoord_1.getCenterYCoord)(config);
    var actualY = "center";
    if (disableVHBounds) {
        return { actualY: actualY, top: Math.max(0, top) };
    }
...
function createAnchoredBottom(config) {
    var vhMargin = config.vhMargin, screenBottom = config.screenBottom, elHeight = config.elHeight, disableSwapping = config.disableSwapping, disableVHBounds = config.disableVHBounds;
    var top = (0, getCoord_1.getBottomCoord)(config);
    var actualY = "bottom";
    if (disableVHBounds || top > vhMargin) {
        return { actualY: actualY, top: top };
    }
    var swappedTop = (0, getCoord_1.getTopCoord)(config);
    if (disableSwapping || swappedTop + elHeight > screenBottom) {
        top = screenBottom - elHeight;
    }
    else {
...
function createAnchoredBelow(config) {
    ...
    var top = (0, getCoord_1.getBelowCoord)(config);
    var actualY = "below";
    if (disableVHBounds || top + elHeight <= screenBottom) {
        return { actualY: actualY, top: top };
    }
    if (preventOverlap) {
        ...
        return {
            ...
            bottom: (0, getViewportSize_1.getViewportSize)("height") - availableTop,
        };
    }
    var swappedTop = (0, getCoord_1.getAboveCoord)(config);
    if (disableSwapping || swappedTop < vhMargin) {
        top = screenBottom - elHeight;
    }
    else {
