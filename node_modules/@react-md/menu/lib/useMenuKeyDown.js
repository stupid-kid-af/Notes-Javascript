...
function useMenuKeyDown(_a) {
    ...
    var items = (0, react_1.useMemo)(function () {
        ...
        return (0, utils_1.getFocusableElements)(menu, true);
    }, [menu]);
    ...
    return (0, utils_1.useKeyboardMovement)(__assign(__assign({}, (horizontal
        ? utils_1.MovementPresets.HORIZONTAL_MENU
        : utils_1.MovementPresets.VERTICAL_MENU)), { focusedIndex: focusedIndex, onChange: function (_a) {
            var index = _a.index;
            setFocusedIndex(index);
            if (items[index]) {
                items[index].focus();
            }
        }, items: items.map(function (item) { return (0, utils_1.extractTextContent)(item); }), onKeyDown: function (event) {
            if (onKeyDown) {
                onKeyDown(event);
            }
            if (event.key === "Escape") {
