...
function handleAddMessage(state, message, duplicates) {
    ...
    if (isNext || isImmediate) {
        ...
        if (isImmediate && current.messagePriority !== "immediate") {
            return __spreadArray([current, message, current], remaining, true);
        }
        return __spreadArray([current, message], remaining, true);
    }
    if (i === -1 || (isDuplicable && isNormal)) {
        return __spreadArray(__spreadArray([], state, true), [message], false);
    }
    if (isNormal) {
        if (isRestart) {
...
function useMessageQueue(_a) {
    ...
    var _e = (0, react_1.useReducer)(function (state, action) { return reducer(state, action); }, defaultQueue), queue = _e[0], dispatch = _e[1];
    var queueRef = (0, react_1.useRef)(queue);
    var addMessageDispatch = (0, react_1.useCallback)(function (message) {
    ...
    var popMessageDispatch = (0, react_1.useCallback)(function () {
        dispatch((0, exports.popMessage)());
    }, []);
    var resetQueueDispatch = (0, react_1.useCallback)(function () {
        dispatch((0, exports.resetQueue)());
        return queueRef.current;
    }, []);
    var _f = (0, utils_1.useToggle)(defaultQueue.length > 0), visible = _f[0], showMessage = _f[1], hideMessage = _f[2];
    var _g = (0, utils_1.useTimeout)(hideMessage, timeout), startTimer = _g[0], stopTimer = _g[1], restartTimer = _g[2];
    ...
    (0, useWindowBlurPause_1.useWindowBlurPause)({
