...
function useFixedPositioning(_a) {
    ...
    var _m = (0, react_1.useState)(), style = _m[0], setStyle = _m[1];
    var _o = (0, react_1.useState)(null), element = _o[0], setElement = _o[1];
    var updateStyle = (0, react_1.useCallback)(function (nextElement) {
        ...
        var opts = __assign({ initialX: initialX, initialY: initialY, xMargin: xMargin, vwMargin: vwMargin, yMargin: yMargin, vhMargin: vhMargin, width: width, transformOrigin: transformOrigin, preventOverlap: preventOverlap, disableSwapping: disableSwapping, disableVHBounds: disableVHBounds, anchor: anchor, container: getFixedTo(fixedTo), element: node }, overrides);
        var _a = (0, utils_1.getFixedPosition)(opts), style = _a.style, actualX = _a.actualX, actualY = _a.actualY;
    ...
    var handleEnter = (0, react_1.useCallback)(function (node, appear) {
        if (onEnter) {
            onEnter(node, appear);
        }
        updateStyle(node);
    }, [onEnter, updateStyle]);
    var handleEntering = (0, react_1.useCallback)(function (node, appear) {
        if (onEntering) {
            onEntering(node, appear);
        }
        updateStyle(node);
    }, [onEntering, updateStyle]);
    var handleEntered = (0, react_1.useCallback)(function (node, appear) {
        if (onEntered) {
            onEntered(node, appear);
        }
        updateStyle(node);
    }, [onEntered, updateStyle]);
    var handleExited = (0, react_1.useCallback)(function (node) {
        if (onExited) {
            onExited(node);
        }
        setElement(null);
    }, [onExited]);
    (0, utils_1.useResizeListener)({
        enabled: !!element,
